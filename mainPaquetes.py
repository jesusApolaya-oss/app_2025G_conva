...
# PDF HELPERS (UPN)
# =========================================================
def _encabezado_pdf(c, titulo_principal, alumno, codigo, carrera_upn, campus, tipo_paquete, logo_path):
    width, height = A4
    ...
    c.drawString(40, y - 42, f"Tipo de Paquete: {str(tipo_paquete).upper()}")
...
def generar_pdf_convalidados(
    ...
    _footer_pdf(c, plan_estudios, 175)
...
# ============================
# BLOQUE 4 / 4
# UI + REPORTES (CORREGIDO: NO SE CONGELA + BORRA DROPDOWNS)
# ============================
def main(page: ft.Page):
    ...
    # ✅ Bandera anti-reentrancia (evita congelado al limpiar)
    is_resetting = False
    ...
    # ✅ Dropdowns con placeholder (value="")
    campus_dd = ft.Dropdown(
        label="Campus / Sede",
        options=[ft.dropdown.Option("", "-- Seleccione --")] + [
            ft.dropdown.Option(s) for s in ["BREÑA","CAJAMARCA","CHORRILLOS","COMAS","LOS OLIVOS","TRUJILLO","SAN JUAN","ATE","VIRTUAL"]
        ],
        value="",
        width=250,
    )

    paquete_dd = ft.Dropdown(
        label="Tipo de Paquete",
        options=[ft.dropdown.Option("", "-- Seleccione --")] + [
            ft.dropdown.Option(s) for s in ["P 1", "P 1 - 2"]
        ],
        value="",
        width=250,
    )
    ...
    carrera_dd = ft.Dropdown(
        label="Carrera",
        options=[ft.dropdown.Option("", "-- Seleccione --")] + [ft.dropdown.Option(c) for c in carreras],
        value="",
        width=400
    )

    unidad_dd = ft.Dropdown(
    ...
    def on_carrera_change(e):
        ...
        unidad_dd.options = [ft.dropdown.Option("", "-- Seleccione --")]
        unidad_dd.value = ""

        if carrera_dd.value and carrera_dd.value != "":
            df_tmp = df_base[df_base["CARRERA"] == carrera_dd.value]
            unidades = sorted(df_tmp["UNID. NEGOCIO"].dropna().unique())
            unidad_dd.options = [ft.dropdown.Option("", "-- Seleccione --")] + [ft.dropdown.Option(u) for u in unidades]
    ...
    def limpiar_click(e):
        ...
        try:
            ...
            page.snack_bar = ft.SnackBar(ft.Text(f"Error al limpiar: {ex}"), bgcolor=ft.Colors.RED)
            page.snack_bar.open = True
            page.update()
        finally:
            is_resetting = False
    ...
    def procesar_click(e):
        ...
        if campus_dd.value in (None, "") or paquete_dd.value in (None, "") or not plan_field.value:
            page.snack_bar = ft.SnackBar(ft.Text("Completa Campus, Tipo de Paquete y Plan de estudios"), bgcolor=ft.Colors.RED)
            page.snack_bar.open = True
            page.update()
            return

        if carrera_dd.value in (None, "") or unidad_dd.value in (None, ""):
        ...
        ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Row([campus_dd, paquete_dd, plan_field]),
